...
这里的 `ollama` 和 `qdrant`, 是 **service 名, 不是容器名**

Compose 的思路是

1. 你先定义一个项目里的服务
2. Compose 再**在这个服务里面帮你创建真正的容器, 网络, volume**
...
`backend` 是项目名 (目录), `ollama` 是 service 名, `backend-ollama-1` 才是真正的容器名
...
`docker` 看的是真实容器名, `docker compose` 看的是当前项目下的 service
...
很多人会想: "可是我自己都不知道容器名, 它怎么知道要把模型装到哪个容器里? "

答案是: 它不是靠"猜容器名"来找目标的, 而是靠 **当前 compose 项目 + service 名** 来找

假如 `docker-compose.yml` 某段代码为
...
### 2.5 那 `docker compose` 创建的容器名是随机的吗

也不是随机的, 如果你不在 `docker-compose.yml` 写 `container_name`, Compose 会按规则自动生成容器名, 通常像这样
...
这个命令适合启动"**当前项目**已经创建过容器, 而且 Compose 还能正确识别到这些容器"的情况

但是它 不会创建新容器, 如果它在当前项目找不到它认为应该存在的容器, 就会报
...
你删掉旧容器之后, 名字冲突解决了, 新容器终于可以创建, 但接下来你发现模型像是没了
