...
class ResolveAction(Enum):
    """Directive returned by a resolver to control the retrieval loop."""

    # Include this result in the context and continue to the next candidate.
    INCLUDE = auto()
    # Drop this result (duplicate doc / load error) but continue scanning.
    SKIP = auto()
    # Drop this result AND stop traversal entirely.
    # Used when the full-doc cap is reached: results ranked below the capped
    # entry are less relevant, so substituting them would degrade context quality.
    STOP = auto()
...
class ResolveResult:
    ...
    # Populated only when action is INCLUDE.
...
class TextResolver:
    ...
    def resolve(self, index: int, doc: Document) -> ResolveResult:
        title, url, source_label = _extract_common(index, doc)
        return ResolveResult(
            action=ResolveAction.INCLUDE,
            item=ContextItem(
                index=index,
                chunk_type="text",
...
class FullDocResolver:
    ...
    Future: replace load_document with a MinIO/object-store fetch using
    ...
    def resolve(
        ...
        *,
        ...
        # Cap reached: signal the loop to stop entirely. Any result ranked below
        # this entry is less relevant than the one we just declined to load, so
        # continuing would substitute a lower-quality result in its place.
        if full_doc_count >= max_full_docs:
            return ResolveResult(action=ResolveAction.STOP)

        # Duplicate: this document body is already in context; skip silently
        # so the loop can continue scanning for chunks from other documents.
        if doc_id in seen_full_doc_ids:
            return ResolveResult(action=ResolveAction.SKIP)
...
# ---------------------------------------------------------------------------
# Singletons — resolvers are stateless; share one instance per type.
# ---------------------------------------------------------------------------
...
def get_resolver(
    meta: dict,
) -> TextResolver | CodeBlockResolver | ImageResolver | FullDocResolver:
