...
from docmind.ingestion.nodes import _strict_split_markdown, _strict_split_pdf
...
def test_strict_split_markdown():
    ...
    chunks = _strict_split_markdown(doc, max_size=100, overlap=0)
    for c in chunks:
...
def test_strict_split_pdf():
    ...
    chunks = _strict_split_pdf(doc, max_size=50, overlap=0)
    for c in chunks:
        print(f"CHUNK: {c.page_content!r}, META: {c.metadata}")
